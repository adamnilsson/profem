...
for i=1:size(ex,1)
    ...
    stress(i,:)=astress;
%     vMs(i,1) = sign(prod(astrain(1:3)))*vMstress(astress); % Used for
%     experiment on determing push or pull for vMises stress.
    vMs(i,1) = vMstress(astress);
end
