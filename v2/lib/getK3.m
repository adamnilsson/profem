function [K, M] = getK3(edof, coord, MAT, D, rho)
K=sparse(size(coord,1)*3,size(coord,1)*3);
M=sparse(size(coord,1)*3,size(coord,1)*3);
...
for i = 1:t
    ...
    if nargout ==2
        M(edof3(i,2:13),edof3(i,2:13)) = M(edof3(i,2:13),edof3(i,2:13)) + soli4m(ex,ey,ez,rho);
    end
end
close(wb)
end